...
def main() -> None:
    ...
    run_step("Etape 1/10 - Data cleaning", run_data_cleaning)  # Je commence par nettoyer les donnees.
    run_step("Etape 2/10 - Minimum Variance 2.1", run_min_var_2_1)  # Je construis ensuite l'investment set.
    run_step("Etape 3/10 - Minimum Variance 2.2", run_min_var_2_2)  # Je calcule ensuite le portefeuille minimum variance.
    run_step("Etape 4/10 - Value-Weighted 2.3", run_value_weighted_2_3)  # Je termine la Part I avec le benchmark value-weighted.
    run_step("Etape 5/10 - Carbon Footprint 3.1", run_carbon_3_1)  # Je mesure ensuite le profil carbone des deux portefeuilles de reference.
    run_step("Etape 6/10 - Minimum Variance Carbon 3.2", run_min_var_carbon_3_2)  # Je construis ensuite la version active avec contrainte carbone a 50%.
    run_step("Etape 7/10 - Tracking Error Carbon 3.3", run_tracking_error_carbon_3_3)  # Je construis ensuite la version passive avec contrainte carbone a 50%.
    run_step("Etape 8/10 - Carbon Comparison 3.4", run_carbon_3_4)  # Je termine la comparaison de la Partie 3.
    run_step("Etape 9/10 - Net Zero 4.1", run_net_zero_4_1)  # Je construis ensuite la strategie net-zero passive.
    run_step("Etape 10/10 - Passive Comparison 4.2", run_passive_comparison_4_2)  # Je termine par la comparaison finale des strategies passives.
